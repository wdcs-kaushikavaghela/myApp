      ...
      if (friendsDeatils) {
        ...
        if (objIndex === -1) {
          ...
          if (filteredArray.length > 0) {
            return mapper.responseMapping(userConstant.CODE.BadRequest , "you have already liked this post")
          }else{
            likesArray.push({
              userName : details.userName
            })
  ...
  if (
    ...
    if (userDeatils) {
      ...
      if (friendsDeatils) {
        ...
        let objIndex = postArray.findIndex((array) => array._id == details.post_id);
        console.log("objIndex", objIndex);
        if (objIndex === -1) {
          return mapper.responseMapping(
            userConstant.CODE.DataNotFound,
...
async function replyToComment(details){
  if(!details || !details.userName || Object.keys(details).length == 0 ){
    return mapper.responseMapping(userConstant.CODE.BadRequest , userConstant.MESSAGE.InvalidDetails)
  }else{
    let userQuery = {
          userName : details.userName,
    ...
    if(userDetails){
      ...
      if(fromUserDetails){
        ...
        let index = userDetails.posts.findIndex((post)=>post._id ==details.post_id)
        ...
        if(index === -1){
          ...
          let commentIndex = commentsArray.findIndex((comment)=>comment._id == details.comment_id)
...
async function getOneUser(id){
  try {
    ...
    if(user){
      ...
      return mapper.responseMappingWithData(userConstant.CODE.Success, userConstant.MESSAGE.Success ,filteredUserResponseFields)
    }else{
      return mapper.responseMapping(
        userConstant.CODE.DataNotFound,
        userConstant.MESSAGE.TemplateNotFound

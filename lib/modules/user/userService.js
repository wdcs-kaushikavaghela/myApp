...
async function getAllUsers(request) {
  try {
    ...
    if (request.id && request.showFriends === 'true') {
  ...
  if (userId != friendId) {
    ...
    if (userDetails) {
      ...
      if (friendsDeatils) {
        ...
        let objIndex = friendRequests.findIndex((array) => array._id == userId);
...
async function dislikePost(details) {
  // let details = req.body;
  if (
    !details ||
    Object.keys(details).length == 0 ||

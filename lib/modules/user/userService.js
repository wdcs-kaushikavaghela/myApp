...
async function addLike(details) {
  ...
  if (
    ...
    if (friendsDeatils) {
      let postsArray = friendsDeatils.posts;
      // console.log("postsArray",postsArray)
      console.log("details._id", typeof details._id);

      let matchedPost = postsArray.filter((array) => array._id == details._id);
      console.log("matchedPost", matchedPost);
      let matchedObj = matchedPost[0];

      matchedObj.likesBy = matchedObj.likesBy.push({
        userName: details.userName,
      });
      matchedObj.likesCount = matchedObj.likesCount + 1;

      console.log("matchedObj", matchedObj.likesBy);
